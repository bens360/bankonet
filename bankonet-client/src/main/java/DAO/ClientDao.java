...
public class ClientDao {
	...
	public static Client recupereClient(String loginClient ,String mdpClient ) {
		...
		try {
			while (rSclient.next()) {
				...
				client = new Client(rSclient.getString("identifiant"), civilite, rSclient.getString("nom"), rSclient.getString("prenom"), mdpClient, loginClient);				
			}
	...
	public static void SoldeDesComptes(Client client) {
		try {
			...
			ResultSet rScomptesList = JDBCConnexionBDD.jDBCRequestBDD(request);
			try {
				while (rScomptesList.next()) {
					CompteCourant compteCourant = new CompteCourant(rScomptesList.getInt("id_compte_courant"), rScomptesList.getString("intitule"), rScomptesList.getDouble("solde"), rScomptesList.getDouble("montant_decouvert_autorise"));
					System.out.println("Le solde du compte: " + compteCourant.getIntitule() + " est de : "+ compteCourant.getSolde());
				}
			} catch (SQLException e) {
